package com.unicred.data.remote // Changed package declaration
...
// import com.unicred.data.StudentWalletResponse // This was an old import for a different StudentWalletResponse, remove if not used elsewhere for that specific type
import com.unicred.data.remote.responses.StudentWalletApiResponse // Correct import for the new data class
...
interface ApiService {
    ...
    // This one likely refers to a different StudentWalletResponse from com.unicred.data
    // For clarity, if the new one is always com.unicred.data.remote.responses.StudentWalletApiResponse, consider aliasing or renaming if both are needed.
    // For now, assuming it might be a different one.
    @GET("/api/students/{id}/wallet")
    suspend fun getStudentWallet(@Path("id") studentId: String): Response<com.unicred.data.StudentWalletResponse> 
    ...
    suspend fun getStudentCredentialsByUsername(@Path("username") username: String): Response<StudentWalletApiResponse> // MODIFIED RETURN TYPE
